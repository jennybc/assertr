...
The first noticeable difference between `verify` and `assert` is that `verify`
...
Notice what happens when we require that `mpg` be within 2 standard deviations
of the mean:
...
with z-scores. However, for most practical purposes, the related predicate generator `within_n_mads` is more useful.
...
to identify them using these statistics. In contrast, 
...
The problem with having to plot data sets to catch anomalies is that it is *really* hard to visualize 4-dimensions at once, and it is near impossible with
...
of their median. They are not, and the pipeline errors out.
...
For an example of an `assert_rows` use case, let's say that we got a data set
...
cosine distances are constrained from -1 to 1, it is easy to use a non-dynamic
...
failed automated checks often send an email to the maintainer of a
...
(this particular `send.mail` formulation will only for gmail recipients;
...
`assertr` is built with robustness, correctness, and extensibility in mind.
...
elements of a vector are within 3 IQRs of the median (IQR = interquartile range). We need to create a function that looks like this
